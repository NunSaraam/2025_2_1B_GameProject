...
public class AchievementManager : MonoBehaviour
{
    ...
    public List<AchievementData> allAchievemunts = new List<AchievementData>();

    [Header("UI References")]
    public GameObject achievementPopupPrefab;
    public Transform popupParent;
    public GameObject achievementPanel;
    public Transform achievementLisContent;
    ...
    private void Start()
    {
        ResetAllAchievements();                         //시작 시에 리셋 강제로 (테스트용) 나중에 배포시에 지움.
    ...
    {
        if (achievementLisContent == null ||  achievementSlotPrefab == null) return;                //두 내용중 하나라도 없으면 오류기 때문에 retutn

        foreach (Transform child in achievementLisContent)
        {
            Destroy(child.gameObject);

        }

        foreach (AchievementData achievement in allAchievemunts)
        {
            GameObject slot = Instantiate(achievementSlotPrefab, achievementLisContent);
    ...
    public void ResetAllAchievements()          //업적 초기화
    {
        foreach (AchievementType type in System.Enum.GetValues(typeof(AchievementType)))
        {
            progressData[type] = 0;
    ...
    public void UpdateProgress(AchievementType type, int amount = 1)                        //진행도 업데이트
    {
        ...
        foreach (AchievementData achievement in allAchievemunts)                            //해당 타입이 모든 업적 체크
        {
